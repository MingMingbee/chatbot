# app.py — 원본 유지 + (1) 줄바꿈 표시 개선 + (2) 선택: BOT_TYPE로 유형 고정 + 유형 배지
import streamlit as st
from openai import OpenAI

st.set_page_config(page_title="연구용 실험 챗봇", page_icon="🤖", layout="centered")

# ── TypeCode 결정: secrets(BOT_TYPE) > URL ?type= > 기본 1  (원본 유지 + 선택 기능)
...
# —— 시스템 프롬프트(사용자 제공 원문 그대로). TypeCode만 주입 —— (원본 그대로)
SYSTEM_PROMPT = f"""
